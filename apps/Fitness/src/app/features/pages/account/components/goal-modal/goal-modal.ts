...
export class GoalModalComponent implements OnInit {
    ...
    selectedGoal = signal<string>("lose_weight");
    goalOptions = signal<{value: string; label: string}[]>([]);

    ngOnInit(): void {
        if (this.dialogConfig.data?.currentGoal) {
            this.selectedGoal.set(this.dialogConfig.data.currentGoal);
        }
    ...
    private updateOptions(): void {
        const options = [
            ...
            {
                value: "gain_more_flexible",
                label: this.translate.instant("ACCOUNT.GOAL.GAIN_MORE_FLEXIBLE"),
            },
            {
                value: "learn_the_basic",
                label: this.translate.instant("ACCOUNT.GOAL.LEARN_THE_BASIC"),
            },
        ];
