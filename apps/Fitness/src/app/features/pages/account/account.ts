        ...
        const sub = this.userService.getLoggedUserData().subscribe({
            error: (error) => {
                this.messageService.add({
                    severity: "error",
                    summary: this.translate.instant("ACCOUNT.MESSAGES.ERROR"),
                    detail: this.translate.instant("ACCOUNT.MESSAGES.ERROR_LOADING_DATA"),
                });
            },
        });
    ...
    openChangePasswordModal(): void {
        ...
        if (dialogRef) {
            const sub = dialogRef.onClose.subscribe(
                (result: {oldPassword: string; newPassword: string}) => {
                    if (result) {
